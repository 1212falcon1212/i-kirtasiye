...
function MarketBrandContent() {
    ...
    const loadBrandInfo = useCallback(async () => {
        try {
            const response = await api.get<{ status: string; data: Brand }>(`/brands/${slug}`);
            if (response.data?.data) setBrandInfo(response.data.data);
        } catch (error) {
                ...
                if (response.data) {
                    ...
                    if (items.length > 0) {
                        ...
                        setAvailableCategories((prev) => {
                            ...
                            categoriesMap.forEach((v, k) => merged.set(k, v));
    ...
    const applyFilters = (next: Filters) => {
        ...
        router.push(`/market/marka/${slug}${qs ? `?${qs}` : ''}`, { scroll: false });
                ...
                <FilterSection title="Kategori">
                    {availableCategories.map((cat) => (
                        <CheckRow
                            ...
                            onChange={(next) => applyFilters({ ...filters, category: next ? cat.slug : '' })}
                        />
                    ))}
                </FilterSection>
            )}
    ...
    return (
        <div className="max-w-[1440px] mx-auto px-6 pt-5">
            ...
            <div className="mb-2 text-[12px]" style={{ color: 'var(--fg-muted)' }}>
                ...
                <span> · </span>
                <Link href="/market/markalar" className="hover:underline" style={{ color: 'var(--fg-muted)' }}>
                    Markalar
                </Link>
                <span> · </span>
                <span style={{ color: 'var(--fg)' }}>{brandName}</span>
            </div>

            {/* Brand header card */}
            <div
                className="mb-5 flex items-center gap-5 rounded-[10px] px-5 py-4"
                style={{
                    background: 'var(--bg-elevated)',
                    border: '1px solid var(--border)',
                }}
            >
                <div
                    className="flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-[8px]"
                ...
                >
                    {brandInfo?.logo ? (
                        ...
                        <Building2 className="h-8 w-8" style={{ color: 'var(--fg-soft)' }} />
                    )}
                </div>
                <div className="flex-1">
                    <h1 className="text-[24px] font-semibold leading-tight">{brandName}</h1>
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <h2 className="text-[20px] font-semibold leading-tight">{brandName} ürünleri</h2>
                    <p className="mt-1 text-[13px]" style={{ color: 'var(--fg-muted)' }}>
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2">
                    {filters.category && (
                        <span className="chip chip-accent" style={{ paddingRight: 4 }}>
                            {availableCategories.find((c) => c.slug === filters.category)?.name || filters.category}
                            <button
                                type="button"
        ...
        <Suspense
            fallback={
                <div className="max-w-[1440px] mx-auto px-6 pt-5">
                    ...
                    <Skeleton className="h-24 w-full mb-5 rounded-[10px]" />
