...
class HomepageSectionResource extends Resource
{
    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                Forms\Components\Section::make('Bölüm Bilgileri')
                    ->schema([
                        ...
                        Forms\Components\Select::make('type')
                            ->label('Bölüm Tipi')
                            ->options(HomepageSection::typeOptions())
                            ->required()
                            ->reactive(),
                    ])->columns(2),
                ...
                Forms\Components\Section::make('Bölüm Ayarları')
                    ->schema([
                        Forms\Components\KeyValue::make('settings')
                            ...
                            ->keyLabel('Anahtar')
                            ->valueLabel('Değer')
                            ->helperText('Örn: limit → 8, category_id → 5')
                            ->columnSpanFull(),
                    ]),
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('type')
                    ->label('Tip')
                    ->badge()
                    ->formatStateUsing(fn(string $state): string => HomepageSection::typeOptions()[$state] ?? $state),
