...
/**
 * Kolaygelsin Kargo Provider (REST API with Token)
 */
class KolaygelsinProvider implements ShippingProviderInterface
{
    protected array $config;
    protected bool $enabled;
    protected string $tokenCacheKey = 'kolaygelsin_token';

    public function __construct()
    {
        $this->config = [
            'api_url' => Setting::getValue('shipping.kolaygelsin_api_url', 'https://api.kolaygelsin.com'),
            'customer_code' => Setting::getValue('shipping.kolaygelsin_customer_code', ''),
            'password' => Setting::getValue('shipping.kolaygelsin_password', '', true),
        ];
        $this->enabled = Setting::getValue('shipping.kolaygelsin_enabled', false);
    }
    ...
    protected function getToken(): ?string
    {
        ...
        try {
            ...
            Log::error('Kolaygelsin token error: ' . $e->getMessage());
            return null;
        }
    }
    ...
    {
        if (!$this->isAvailable()) {
            return ShipmentResult::failure('Kolaygelsin entegrasyonu aktif değil.');
        }

        $token = $this->getToken();
        if (!$token) {
            return ShipmentResult::failure('Kolaygelsin API token alınamadı.');
        }
    ...
    public function trackShipment(Order $order): TrackingResult
    {
        $token = $this->getToken();
        if (!$token) {
            return TrackingResult::failure('Kolaygelsin API token alınamadı.');
        }
